...
*
      ...
      dimension aa(npamx,npamx),bb(npamx)
...
c          zc(i)   = zce(i)
          ...
          ty(i)   = tmy(i)
          tz(i)   = tmz(i)
c          write(71,'(i6,10d15.7)') i,yc(i),zc(i),
...
c          zc(mt+i-mb)  = zce(i)
          ...
          ty(mt+i-mb)= tmy(i)
          tz(mt+i-mb)= tmz(i)
c          write(72,'(i6,10d15.7)') mt+i-mb,yc(mt+i-mb),zc(mt+i-mb),
...
c          zc(mt+m+(i-n))   = zcsl(i)
          ...
          ty(mt+m+(i-n))   = tmysl(i)
          tz(mt+m+(i-n))   = tmzsl(i)
c         write(82,'(i6,10d15.7)') mt+m+i-n,yc(mt+m+i-n),zc(mt+m+i-n),  
...
c            write(98,*) 'eccolo1!', k,nt+n,jt,ph(k)
            ryk  = ry(k)
            rzk  = rz(k)
c            write(98,*) 'ph rn',ph(k),ryk,rzk 
            bik  = ryk*gik
            cik  = rzk*gik
            dik  = gik*( ryk*(xik-xisk) - rzk*(zek-zesk) )  
            eik  = gik*( ryk*(zek-zesk) + rzk*(xik-xisk) )   
...
c        aa(iii,i1)         = aa(iii,i1)         + drl
c        aa(iii,iij)        = aa(iii,iij)        + rli  
c        aa(iii,iij-1)      = aa(iii,iij-1)      - rli1 
c        aa(iii,iij+mn)     = aa(iii,iij+mn)     + rli*(xii1-xisi) 
c        aa(iii,iij-1+mn)   = aa(iii,iij-1+mn)   - rli1*(xii1-xisi1) 
c        aa(iii,iij+2*mn)   = aa(iii,iij+2*mn)   + rli*(zei1-zesi) 
c        aa(iii,iij-1+2*mn) = aa(iii,iij-1+2*mn) - rli1*(zei1-zesi1) 
c        aa(iii,iij+3*mn)   = aa(iii,iij+3*mn)   + 
c     #                       0.5d0*rli*((xii1-xisi)**2-(zei1-zesi)**2)
c        aa(iii,iij-1+3*mn) = aa(iii,iij-1+3*mn) - 
c     #                      0.5d0*rli1*((xii1-xisi1)**2-(zei1-zesi1)**2)
c        aa(iii,iij+4*mn)   = aa(iii,iij+4*mn)   +
c     #                      rli*(xii1-xisi)*(zei1-zesi)
c        aa(iii,iij-1+4*mn) = aa(iii,iij-1+4*mn) - 
c     #                      rli1*(xii1-xisi1)*(zei1-zesi1)
...
c        aa(iii+k1,iij)       = aa(iii+k1,iij)        + rli  
c        aa(iii+k1,iij-1)     = aa(iii+k1,iij-1)      - rli1 
c        aa(iii+k1,iij+mn)    =aa(iii+k1,iij+mn)    +rli*(xij-xisi) 
c        aa(iii+k1,iij-1+mn)  =aa(iii+k1,iij-1+mn)  -rli1*(xij-xisi1)
c        aa(iii+k1,iij+2*mn)  =aa(iii+k1,iij+2*mn)  +rli*(zej-zesi) 
c        aa(iii+k1,iij-1+2*mn)=aa(iii+k1,iij-1+2*mn)-rli1*(zej-zesi1)
c        aa(iii+k1,iij+3*mn)  =aa(iii+k1,iij+3*mn)  + 
c     #                       0.5d0*rli*((xij-xisi)**2-(zej-zesi)**2)
c        aa(iii+k1,iij-1+3*mn) = aa(iii+k1,iij-1+3*mn) - 
c     #                      0.5d0*rli1*((xij-xisi1)**2-(zej-zesi1)**2)
c        aa(iii+k1,iij+4*mn)   = aa(iii+k1,iij+4*mn)   +
c     #                           rli*(xij-xisi)*(zej-zesi)
c        aa(iii+k1,iij-1+4*mn) = aa(iii+k1,iij-1+4*mn) - 
c     #                           rli1*(xij-xisi1)*(zej-zesi1)
c        ff(iii+ k1 )  = ff(iii+k1)   + 0.d0 
c        ff(iii+ k1 )  = ff(iii+k1)   - drl*ph(j)
* 
* - phi sulla SL  (j)
...
c        write(98,*) 'eccolo10!',i1,nt+n,jt
c        write(98,*)  xii1,zei1,ph(i1)
...
c        write(98,*) 'eccolo11!',i,nt+n,jt
c        write(98,*) xii,zei,ph(i)
...
c      write(90,*)
c      write(90,*)
c      write(91,'(i6,2d15.6)') i,yc(i),bb(i)
c      enddo
c      write(91,*)
c      write(91,*)
c      endif
...
ccc      write(88,*) '# jt ',jt
      do i = mt+1,mt+m
        ...
        d(j) = d(i)
        e(j) = e(i)
ccc        write(88,'(i6,5d15.7)') i,a(i),b(i),c(i),d(i),e(i)
      enddo
ccc      write(88,*)
      do i = nt+1,nt+n
        j  = ntt-(i-nt)+1
