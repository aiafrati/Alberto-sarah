...
*
      ...
      dvol    = volb0 + volj 
      do i = 1, npt
        dvol = dvol + (yv(i+1)-yv(i))*(zv(i+1)+zv(i))/2.d0
      end do
      cm     = 0.d0
      cq     = 0.d0
      call izero(kint,yi,zi,yv,zv,1,npc,cm,cq)
      if(kint.eq.-999)then
        write(*,*) '   OKKIO, check non trova intersezione;' 
        write(*,*) '   prendo kint=npc.'
        kint= npc
        yi  = yv(npc+1)
        zi  = zv(npc+1)
      endif
...
*          call wigl(alpha,al,bl,tl,xsec,ygn,zgn,rnxg,ng,dztr)
          call savan(al,bl,alpha,dztr,zmaxs,xsec,ygn,zgn,rnxg,ng)
        elseif(iwig.eq.2)then
*          call wigl(alpha,al,bl,tl,xsec,ygn,zgn,rnxg,ng,dztr)
          call geopla(xsec,dztr,alpha,ygn,zgn,rnxg,ng,al,bl,tl)
        endif
* 
*        write(36,*) '# ',jt 
*        do i=1,ng
